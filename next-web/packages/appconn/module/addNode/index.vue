        ...
        >
          ...
          <FFormItem label="跳转类型" prop="jumpType">
            <FRadioGroup v-model="nodeForm.jumpType">
              <FRadio :value="1"> 内部节点 </FRadio>
              <FRadio :value="2"> 外部节点 </FRadio>
            </FRadioGroup>
          </FFormItem>
...
import { FMessage, FPopper, FButton, FInputNumber } from '@fesjs/fes-design';
...
const fetchTableData = async () => {
  ...
  try {
    const data = await queryAttribute({ nodeId: nodeId.value });
...
const saveVerificationRule = async (index) => {
  try {
    ...
    validateIdList.value.push(res.id);
  } catch (err) {
    console.warn(err);
  }
};
...
const submitAttributeAndRule = async () => {
  try {
    if (attributeOption.value === '内置') {
      await nodeBindAttribute({ nodeId: nodeId.value, uiId: uiId.value });
      showDrawer.value = false;
    } else {
      await attributeFormRef.value.validate();
      const params = { ...attributeForm.value };
      delete params.isVerification;
      uiId.value = (await saveNodeAttribute(params)).id;
      if (attributeForm.value.isVerification) {
        validateIdList.value.forEach(async (item) => {
          await ruleBindAttribute({ uiId: uiId.value, validateId: item });
        });
      }
      showDrawer.value = false;
    }
    FMessage.success('成功添加属性信息');
...
const unbindAttribute = async (row) => {
  try {
    await nodeUnbindAttribute({ nodeId: nodeId.value, uiId: row.id });
