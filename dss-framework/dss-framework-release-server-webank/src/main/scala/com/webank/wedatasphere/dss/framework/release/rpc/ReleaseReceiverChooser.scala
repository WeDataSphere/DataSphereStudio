...
class ReleaseReceiverChooser extends ReceiverChooser {
  ...
  override def chooseReceiver(event: RPCMessageEvent): Option[Receiver] = event.message match {
    case _: RequestReleaseOrchestration => receiver
    case _: RequestReleaseOrchestrationStatus => receiver
    case _: RequestPublishHistory => receiver
    case _: RequestPublishUser => receiver
    case _ => None
  }
}
