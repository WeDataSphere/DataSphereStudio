...
class ReleaseReceiver(releaseService:ReleaseService) extends Receiver with Logging{
  ...
  override def receiveAndReply(message: Any, sender: Sender): Any = message match {
    //发布调度
    case requestReleaseOrchestration: RequestReleaseOrchestration =>
      releaseService.releaseOrchestrator(requestReleaseOrchestration)
    //获取发布状态
    case requestReleaseStatus: RequestReleaseOrchestrationStatus =>
      releaseService.getStatus(requestReleaseStatus.getId)
    // 获取发布历史
    case requestPublishHistory: RequestPublishHistory =>
      releaseService.getReleaseHistory(requestPublishHistory)
    // 获取对应的编排模式发布人
    case requestPublishUser: RequestPublishUser =>
      releaseService.getReleaseUserList(requestPublishUser)
  }
