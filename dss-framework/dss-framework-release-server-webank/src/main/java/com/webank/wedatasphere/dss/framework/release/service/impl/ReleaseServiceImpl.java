...
import com.webank.wedatasphere.dss.framework.release.job.BatchReleaseReleaseJob;
...
public class ReleaseServiceImpl implements ReleaseService {
    ...
    public ResponseConvertOrchestrator releaseOrchestrator(RequestReleaseOrchestration request) {
        List<Long> requestOrcIds = request.getOrcIds();
        String releaseUser = request.getUserName();
        if (CollectionUtils.isNotEmpty(requestOrcIds)) {
            //去重
            requestOrcIds=requestOrcIds.stream().distinct().collect(Collectors.toList());
            ...
            Map<Long, List<OrchestratorEntity>> projectMap = new HashMap<>();
            for (Long flowId : requestOrcIds) {
                OrchestratorInfo orchestratorInfo = releaseEnv.getOrchestratorMapper().getOrcInfoByAppId(flowId);
                ...
                //3.查询编排模式是否已经发布过
                OrchestratorEntity orchestratorEntity = new OrchestratorEntity(orchestratorId, orchestratorVersionId, orchestratorName);
                orchestratorEntity.setProjectInfo(projectInfo);
                if (projectMap.containsKey(projectInfo.getProjectId())) {
                    List<OrchestratorEntity> orchestratorEntities = projectMap.get(projectInfo.getProjectId());
                    orchestratorEntities.add(orchestratorEntity);
                } else {
                    List<OrchestratorEntity> orchestratorEntities = new ArrayList<>();
                    orchestratorEntities.add(orchestratorEntity);
                    projectMap.put(projectInfo.getProjectId(), orchestratorEntities);
                }
            }
            BatchReleaseReleaseJob job =null;
            for (Map.Entry<Long, List<OrchestratorEntity>> entry : projectMap.entrySet()) {
                job = new BatchReleaseReleaseJob();
                job.setReleaseUser(releaseUser);
                List<OrchestratorEntity> orchestratorEntityList = entry.getValue();
                ...
                for (OrchestratorEntity entity : orchestratorEntityList) {
                    ...
                    ReleaseTask releaseTask = this.releaseEnv.getTaskService().addReleaseTask(releaseUser,
                            projectInfo.getProjectId(),
                            entity.getOrchestratorId(),
                            entity.getOrchestratorVersionId(), entity.getOrchestratorName(), request.getComment(), Sender.getThisInstance());
                    entity.setReleaseTask(releaseTask);
                    job.setReleaseTask(releaseTask);
                    job.setStatus(ReleaseStatus.RUNNING);
                    releaseEnv.getReleaseJobListener().onJobStatusUpdate(job);
                }
                job.setOrchestratorEntityList(orchestratorEntityList);
                job.setProjectId(entry.getKey());
            ...
            Long lastJobId = job == null ? -999L : job.getJobId();
            return  new ResponseConvertOrchestrator(serializeId(lastJobId),ResponseOperateOrchestrator.inited());
        } else {
            ...
            List<DSSLabel> dssLabels = LabelBuilderFactoryContext.getLabelBuilderFactory().getLabels(request.getLabels());
            Workspace workspace = (Workspace) request.getWorkspace();
            LOGGER.info("trace workflow publish,flowId:{},orchestratorId:{}.收到工作流发布任务 request releaseUser is {}.",
                     request.getOrcAppId(),orchestratorId,releaseUser );
            ...
            //4.生成任务,然后放入到线程池中
            LOGGER.info("trace workflow publish,flowId:{},orchestratorId:{}.开始启动发布任务 request releaseUser is {}.",
                    request.getOrcAppId(),orchestratorId,releaseUser );
