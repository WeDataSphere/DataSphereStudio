...
public class CommonReleaseJobListener implements ReleaseJobListener {
    ...
    public void onJobStatusUpdate(AbstractReleaseJob releaseJob) {
        ...
        String errmsg = releaseJob.getReleaseTask().getErrorMsg();
        if (StringUtils.isNotBlank(errmsg) && errmsg.length() > ReleaseConstant.RELEASE_ERRORLOG_MAXSIZE.getValue()) {
            errmsg = errmsg.substring(0, ReleaseConstant.RELEASE_ERRORLOG_MAXSIZE.getValue());
            releaseJob.getReleaseTask().setErrorMsg(errmsg);
        }
    ...
    public void onJobSucceed(AbstractReleaseJob releaseJob) {
        ...
        releaseJob.getReleaseTask().setErrorMsg("");
        Arrays.stream(releaseHooks).forEach(releaseHook -> releaseHook.postRelease(releaseJob));
    }

    @Override
    public void onJobFailed(AbstractReleaseJob releaseJob, String errorMsg, String logMsg, String bak) {
        releaseJob.setStatus(ReleaseStatus.FAILED);
        releaseJob.setErrorMsg(errorMsg);
        releaseJob.setProgress(1.0);
        releaseJob.getReleaseTask().setErrorMsg(errorMsg);
        releaseJob.getReleaseTask().setBak(bak);
        releaseJob.getReleaseTask().setLogMsg(logMsg);
        onJobStatusUpdate(releaseJob);
        Arrays.stream(releaseHooks).forEach(releaseHook -> releaseHook.postRelease(releaseJob));
    }
