...
public class WebankDSSFrameworkProjectRestfulApi {
    private static final Logger LOGGER = LoggerFactory.getLogger(WebankDSSFrameworkProjectRestfulApi.class);

    @Autowired
    private WebankDSSProjectService webankDSSProjectService;
    @Autowired
    ...
    public Message copyProject(@RequestBody ProjectCopyRequest projectCopyRequest) throws Exception {
        ...
        Workspace sourceWorkspace=SSOHelper.getWorkspace(request);
        Workspace targetWorkspace;
        Long targetWorkspaceId = projectCopyRequest.getTargetWorkspaceId();
        Long sourceWorkspaceId=projectCopyRequest.getWorkspaceId();
        //跨工作空间复制
        if(targetWorkspaceId!=null && !targetWorkspaceId.equals(sourceWorkspaceId) ){
            targetWorkspace = new Workspace();
            String workspaceName = dssWorkspaceService.getWorkspaceName(targetWorkspaceId);
            if(workspaceName==null){
                LOGGER.error("目标工作空间不存在");
                return Message.error("目标工作空间不存在");
            }
            targetWorkspace.setWorkspaceId(targetWorkspaceId);
            targetWorkspace.setWorkspaceName(workspaceName);
            SSOHelper.addWorkspaceInfo(request, targetWorkspace);
            targetWorkspace.addCookie(WORKSPACE_NAME_COOKIE_KEY, workspaceName);
            targetWorkspace.addCookie(WORKSPACE_ID_COOKIE_KEY,targetWorkspaceId.toString());
        }else{
            //同工作空间复制
        ...
        DSSProjectVo projectVo;
        boolean projectIsWhite = false;
        try {
            ...
            projectVo = webankDSSProjectService.copyProject(projectCopyRequest, username,proxyUser, sourceWorkspace,
                    targetWorkspace);

            List<ProjectOrchestratorWhite> whiteList  = projectOrchestratorWhiteService.getWhiteListByProjectId(projectCopyRequest.getProjectId());
            projectIsWhite = CollectionUtils.isNotEmpty(whiteList);

            boolean isWhite =  projectOrchestratorWhiteService
                    .checkProjectAndOrchestratorIsWhite(projectCopyRequest.getProjectId(),0L);

            // 如果复制的项目(所有的编排)已添加到白名单,则复制后的项目也添加到白名单
            if(isWhite){
                ProjectOrchestratorWhite projectWhite = new ProjectOrchestratorWhite();
                projectWhite.setProjectName(projectVo.getName());
                projectWhite.setProjectId(projectVo.getId());
                projectWhite.setOrchestratorName("*");
                projectWhite.setOrchestratorId(0L);
                projectWhite.setCreateBy(username);
                projectWhite.setUpdateBy(username);
                projectOrchestratorWhiteService.addProjectOrchestratorWhite(projectWhite);
            }
        ...
        Message message = Message.ok("复制工程已经成功，工作流正在后台复制中...").data("projectId", projectVo.getId()).data("isWhite", projectIsWhite);
        return message;
    }
    ...
    public Message handoverWorkflows( @RequestBody ReleaseHandoverProjectByOpsRequest releaseHandoverProjectByOpsRequest) {
        ...
        try {
            ...
            if (type.equals(HandOverTypeEnum.ONE.getValue()) || type.equals(HandOverTypeEnum.TWO.getValue()) || type.equals(HandOverTypeEnum.THREE.getValue())) {
                if ("1".equals(type)) {
                    //type=1,基于用户级别交接
                    Map<String, String> reMap = webankDSSProjectService.handoverWorkflowsOnUserLevel(username, recipient, workspace);
                    ...
                    Map<String, String> reMap = webankDSSProjectService.handoverWorkflowsOnWorkspaceLevel(handInfo, username, recipient, workspace);
                    response = Message.ok().data("result", "success").data("errorInfo", reMap.get("errorInfo"));
                    ...
                    Map<String, String> reMap = webankDSSProjectService.handoverWorkflowsOnProjectLevel(username, recipient.trim(), handInfo, workspace);
                    response = Message.ok().data("result", "success").data("errorInfo", reMap.get("errorInfo"));
                }
            } else {
    ...
    public Message operateRecordList(@RequestParam("projectId") Long projectId,
        ...
        try{
            ...
            PageInfo<ProjectOperateRecordBO> pageInfo =
                    webankDSSProjectOperateService.queryRecords(projectId, pageSize, currentPage, recordId,
    ...
    public void downloadResourceOfRecord(@RequestParam("recordId") String recordId, HttpServletResponse httpServletResponse) {
        try {
            String username = SecurityFilter.getLoginUsername(request);
            String projectName=Optional.ofNullable( webankDSSProjectOperateService.getRecordProjectName(recordId)).orElse(recordId);
            httpServletResponse.addHeader("Content-Disposition", String.format("attachment;filename=%s.zip",projectName+"-prod"));
            OutputStream outputStream = httpServletResponse.getOutputStream();
            BmlResource bmlResource=webankDSSProjectOperateService.getExportResultByRecordId(recordId);
    ...
    public  Message batchExportOrchestrators(@RequestBody BatchExportOrchestratorsRequest batchExportOrchestratorsRequest){
        ...
        try {
            ...
            String recordId= webankDSSProjectService.exportOrchestrators(batchExportOrchestratorsRequest, username,proxyUser, workspace);
    ...
    public Message batchImportOrchestrators( @RequestParam( "importType") String importType,
        ...
        if(bmlResource!=null) {
            Consumer<OrchestratorBatchImportInfo> addPriv = importResult -> importResult.getTo().forEach(DSSExceptionUtils.handling(info -> {
                ...
                webankDSSOrchestratorService.setOrchestratorPriv(username, (int) workspace.getWorkspaceId(), projectInfo.getId(),
                        projectInfo.getProjectName(), info.getOrchestratorId().intValue(), accessUsers, 1, workspace); // 1 表示公开权限
            }));
            try {
                webankDSSProjectService.importOrchestrators(projectInfo, bmlResource, username, proxyUser, checkCode,
    ...
    public Message publishOrchestratorsDirectly(@NotNull(message = "发布工程id不能为空") @RequestBody PublishWholeProjectRequest publishWholeProjectRequest) {
        ...
        webankDSSProjectService.publishOrchestratorsDirectly(convertOrchestrationRequest,proxyUser);
