...
public class ECConfTemplateServiceImpl implements ECConfTemplateService {
    ...
    private final static String SPARK_CONF = "spark.conf";


    private final static String CREATOR_KEY = "linkis.entrance.creator.job.concurrency.limit";
    ...
    public List<String> getEngineTypeList(String applicationFilter, String operateUser) {
        if (applicationFilter == null) {
            //get all engine type
            List<String> engineTypes = resourceManageClient.getEngineTypes(operateUser);
            return engineTypes;
        } else {
            return resourceManageClient.getApplicationEngineTypeCategory(operateUser)
                    .stream()
        ...
        return
                resourceManageClient.getTemplateParamConf(engineType,operateUser).stream()
                        .map(e -> {
                            ...
                            BeanUtils.copyProperties(e, confItem);
                            return confItem;
                        }).filter(confItem->!CREATOR_KEY.equals(confItem.getKey()))
                        .collect(Collectors.toList());
    }
    ...
    public PageInfo<ECConfTemplate> getTemplateList(ECConfTemplateGetRequest request) {
        ...
        if("*".equals(engineName)){
            request.setEngineType(engineName);
        }else if(engineName!=null&&engineType==null){
    ...
    public ECConfTemplate saveTemplate(ECConfTemplate ecConfTemplate, Long workspaceId, String operator) {
        ...
        String sparkConfValue = ecConfTemplate.getParamDetails().stream()
                .filter(item -> SPARK_CONF.equals(item.getKey()))
                .map(ECConfItem::getConfigValue)
                .filter(Objects::nonNull)
                .findFirst().orElse(null);

        if(!StringUtils.isEmpty(sparkConfValue)){
            String sparkConfValueTrim = sparkConfValue.replaceAll("\\s", "");
            Optional<ECConfItem> optionalECConfItem = ecConfTemplate.getParamDetails().stream()
                    .filter(item -> sparkConfValueTrim.contains(item.getKey() + "="))
                    .findFirst();
            if(optionalECConfItem.isPresent()){
                throw new DSSRuntimeException("Saved key is duplicated with the spark conf key , key :" + optionalECConfItem.get().getKey());
            }
        }
        ...
        if(Boolean.TRUE.equals(ecConfTemplate.getDefaultForAISQL())){
            ecConfigTemplateMapper.updateOtherTemplatesDefaultForAISQL(workspaceId, ecConfTemplate.getTemplateId());
        }
    ...
    public PageInfo<String> getPermissionUserList(TemplatePermissionUsersGetRequest request) {
        ...
        if (templateDO.getPermissionType() == ECConfTemplate.SPECIFIED_USER_PERMISSION_TYPE) {
            ...
            return dssWorkspaceUserService.getAllWorkspaceUsersPage(templateDO.getWorkspaceId(), request.getPageNow()
                    , request.getPageSize(),request.getUsername()==null);
        }
    }
