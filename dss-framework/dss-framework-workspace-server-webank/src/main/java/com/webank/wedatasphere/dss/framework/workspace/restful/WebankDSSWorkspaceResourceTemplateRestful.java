...
@RestController
public class WebankDSSWorkspaceResourceTemplateRestful {
    @Autowired
    ...
    public Message getConfTemplateList(@Valid ECConfTemplateGetRequest request){
        try {
            ...
            Set<Long> workspaceIds = dssWorkspaceUserService.getUserWorkspaceIds(username).stream().map(Long::valueOf).collect(Collectors.toSet());
    ...
    public Message saveConfTemplateApplyRule(@RequestBody ECConfTemplateApplyRuleSaveRequest request){
        ...
        &&(request.getPermissionType() == ECConfTemplateApplyRule.ALL_USER_PERMISSION_TYPE
                || request.getRuleType() == ECConfTemplateApplyRule.RULE_TYPE_NEWUSER)) {

            return Message.error("" + workspaceName + "是系统默认工作空间，默认工作空间内不允许对所有用户或新用户执行规则");
        }
