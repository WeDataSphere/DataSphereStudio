...
public class ECConfTemplate {
     ...
     * 是否为AI SQL的默认模板
     */
    private Boolean defaultForAISQL;
    ...
    public Boolean getDefaultForAISQL() {
        return defaultForAISQL;
    }

    public void setDefaultForAISQL(Boolean defaultForAISQL) {
    ...
    public static ECConfTemplate fromDO(ECConfigTemplateDO templateDO){
        ...
        if(Integer.valueOf(1).equals( templateDO.getDefaultForAISQL())){
            template.setDefaultForAISQL(true);
        }else{
            template.setDefaultForAISQL(false);
        }
        return template;
    }
    public ECConfigTemplateDO toDO(Long workspaceId){
        ...
        if(Boolean.TRUE.equals( this.defaultForAISQL)){
            templateDO.setDefaultForAISQL(1);
        }else{
            templateDO.setDefaultForAISQL(0);
        }
