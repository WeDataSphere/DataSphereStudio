...
public class ECConfTemplateApplyRule {
    ...
    public static int SPECIFIED_DEPARTMENT_PERMISSION_TYPE=3;
    ...
    private List<PermissionDepartment> permissionDepartments;
    ...
    public ECConfTemplateApplyRule(ECConfTemplate template, int ruleType,String engineName, int permissionType, List<PermissionUser> permissionUsers, List<PermissionDepartment> permissionDepartments, String application,String creator) {
        ...
        if(permissionType==SPECIFIED_DEPARTMENT_PERMISSION_TYPE) {
            this.permissionDepartments = permissionDepartments==null? Collections.emptyList():permissionDepartments;
        }
    ...
    public List<ECConfigTemplateApplyRuleDepartmentDO> getDepartmentDOList(Long workspaceId){
        if(permissionType!=SPECIFIED_DEPARTMENT_PERMISSION_TYPE){
            throw  new DSSRuntimeException("only specified department permission type has department list");
        }
        return
                permissionDepartments.stream().map(department ->
                        new ECConfigTemplateApplyRuleDepartmentDO(department.getName(), getRuleId(), workspaceId)
                ).collect(Collectors.toList());
    ...
    public List<PermissionDepartment> getPermissionDepartments() {
        return permissionDepartments;
    }

    public void setPermissionDepartments(List<PermissionDepartment> permissionDepartments) {
        this.permissionDepartments = permissionDepartments;
    }
}
