...
import com.webank.wedatasphere.dss.framework.workspace.dao.WebankDSSWorkspaceEcReleaseImsRecordMapper;
...
public class ECReleaseSummaryInfoNotificationServiceImpl implements ECReleaseSummaryInfoNotificationService {
    ...
    @Autowired
    private WebankDSSWorkspaceEcReleaseImsRecordMapper imsRecordMapper;

    @Autowired
    private WebankDSSWorkspaceEcKillHistoryMapper webankDSSWorkspaceEcKillHistoryMapper;
    ...
    private final static String DATE_FORMAT_PATTERN = "yyyy-MM-dd HH:mm:00";
    ...
    public boolean tryNotify(ECReleaseStrategy releaseStrategy) {
        ...
        //统计指定间隔时间内的ec kill记录
        List<ECKillSumHistoryRecordDO> ecKillSumHistoryRecordDOS = webankDSSWorkspaceEcKillHistoryMapper.sumECKillInfoByStrategyId(releaseStrategy.getStrategyId(), imsConf.getDuration());
        ...
        //起止时间
        String endTime = new SimpleDateFormat(DATE_FORMAT_PATTERN).format(System.currentTimeMillis());
        String startTime = new SimpleDateFormat(DATE_FORMAT_PATTERN).format(System.currentTimeMillis() - imsConf.getDuration() * 1000 * 60L);
        //工作空间id
        ...
        String crossClusterFlag = releaseStrategy.getCrossCluster() ? "是" : "否";
        ...
        //发送发告警
        ImsAlter imsAlter = new ImsAlter(queue + "队列空闲引擎释放告警", buildImsContent(ecKillSumHistoryRecordDOS, queue, startTime, endTime,crossClusterFlag), ImsWarnTypeEnum.getCodeByType(imsConf.getLevel()),String.join(",", imsConf.getReceiver()));
        try {
    ...
    private String buildImsContent(List<ECKillSumHistoryRecordDO> ecKillSumHistoryRecordDOS, String queue, String startTime, String endTime,String crossClusterFlag){
        StringBuilder sb = new StringBuilder("");
        sb.append("队列名称:").append(queue).append(",跨集群：").append(crossClusterFlag).append("，\n");
        sb.append("告警周期内请求释放的引擎信息如下：\n");
        sb.append("周期起止时间：").append(startTime).append("~").append(endTime).append("\n");
        sb.append("|用户|kill引擎数|释放CPU数|释放内存数（GB）|引擎最大空闲时长（秒）|\n");
        ecKillSumHistoryRecordDOS.forEach(item ->{
            ...
            sb.append(item.getUnlockDurations()/1000).append("|");
            sb.append("\n");
        });
        return sb.toString();
    }
