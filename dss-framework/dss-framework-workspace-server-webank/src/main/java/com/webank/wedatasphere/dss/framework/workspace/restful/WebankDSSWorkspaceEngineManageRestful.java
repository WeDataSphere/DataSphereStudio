...
import com.webank.wedatasphere.dss.common.server.utils.TransformSizeUtil;
...
@RestController
public class WebankDSSWorkspaceEngineManageRestful {
    private static final Logger LOGGER = LoggerFactory.getLogger(WebankDSSWorkspaceEngineManageRestful.class);
    ...
    @Autowired
    WebankDSSWorkspaceECService webankDSSWorkspaceECService;

    @Autowired
    WebankDSSWorkspaceECKillHistoryService webankDSSWorkspaceECKillHistoryService;
    ...
    public Message listEngineConnInstances(@RequestBody ECInstanceRequest ecInstanceRequest){
        ...
        List<ECInstance> ecInstanceList = new ArrayList<>();
        try {
            ecInstanceRequest.setCrossCluster(true);
            List<ECInstance> ecInstanceListCross = resourceManageClient.fetchECInstance(ecInstanceRequest,userName);
            ecInstanceList.addAll(ecInstanceListCross);
            ecInstanceRequest.setCrossCluster(false);
            List<ECInstance> ecInstanceListNotCross = resourceManageClient.fetchECInstance(ecInstanceRequest,userName);
            ecInstanceList.addAll(ecInstanceListNotCross);
            //内存排序，对全量数据排序
    ...
    public Message killEngineConnInstances(@RequestBody KillECInstanceRequest killECInstanceRequest){
        ...
        try {
            ...
            ecInstanceRequest.setCrossCluster(false);
            List<ECKillHistoryRecord> killHistoryRecords = new ArrayList<>(ecInstanceNames.size());
            //必须在kill之前获取ec的信息，否则获取的信息是不准确的。
            List<ECKillHistoryRecord> killHistoryRecordsNotCross = resourceManageClient.fetchECInstance(ecInstanceRequest, userName).stream()
                    .map(e -> ECKillHistoryRecord.convertECInstance2ECKillHistoryRecord(e, workspaceId, "0", userName))
                    .collect(Collectors.toList());
            killHistoryRecords.addAll(killHistoryRecordsNotCross);
            if(killHistoryRecords.size() != ecInstanceNames.size()){
                ecInstanceRequest.setCrossCluster(true);
                List<ECKillHistoryRecord> killHistoryRecordsCross = resourceManageClient.fetchECInstance(ecInstanceRequest, userName).stream()
                        .map(e -> ECKillHistoryRecord.convertECInstance2ECKillHistoryRecord(e, workspaceId, "0", userName))
                        .collect(Collectors.toList());
                killHistoryRecords.addAll(killHistoryRecordsCross);
            }
            //第一步，发起kill
            ...
            if (!killHistoryRecords.isEmpty()) {
                ...
                webankDSSWorkspaceECKillHistoryService.batchAddEcKillRecord(killHistoryRecords);
            }
        }catch (DSSRuntimeException e){
    ...
    public Message getQueueList(@RequestParam ("workspaceId")Long workspaceId,@RequestParam("crossCluster") Boolean crossCluster){
        String userName=  SecurityFilter.getLoginUsername(httpServletRequest);
        ...
        try {
            List<String> queueList = webankDSSWorkspaceECService.getQueueList(workspaceId,crossCluster);
    ...
    public Message getEcReleaseStrategyList(@RequestParam ("workspaceId")Long workspaceId){
        ...
        try {
            List<ECReleaseStrategy> strategies = webankDSSWorkspaceECService.listEcReleaseStrategy(workspaceId);
            return Message.ok()
    ...
    public Message saveEcReleaseStrategy(@RequestBody ECReleaseStrategy strategy){
        ...
        try {
            ECReleaseStrategy savedStrategy = webankDSSWorkspaceECService.saveEcReleaseStrategy(strategy, workspaceId, userName);
    ...
    public  Message deleteECReleaseStrategy(@PathVariable("strategyId")String strategyId){
        ...
        try {
            ECReleaseStrategy deleteResult= webankDSSWorkspaceECService.deleteEcReleaseStrategy(strategyId);
            AuditLogUtils.printLog(userName, workspaceId.toString(), workspaceName, TargetTypeEnum.EC_KILL_STRATEGY,
                    strategyId,
    ...
    public Message changeEcReleaseStrategyStatus(@RequestBody ECReleaseStrategyStatusChangeRequest changeRequest){
        ...
        try {
            ECReleaseStrategy strategy= webankDSSWorkspaceECService.changeEcReleaseStrategyStatus(changeRequest);
            AuditLogUtils.printLog(userName, workspaceId.toString(), workspaceName, TargetTypeEnum.EC_KILL_STRATEGY,
    ...
    public Message listEcKillHistory(@RequestParam("workspaceId")Long workspaceId,@RequestParam("pageNow") int pageNow,
        ...
        try {
            PageInfo<ECKillHistoryRecord> pageInfo = webankDSSWorkspaceECKillHistoryService.listEcKillHistory(workspaceId, pageNow, pageSize);
...
//                        Double m2 = Double.valueOf(memory2.substring(0, memory2.indexOf('G')).trim());
                        Long m1 = TransformSizeUtil.parseByteString(memory1);
                        Long m2 = TransformSizeUtil.parseByteString(memory2);
