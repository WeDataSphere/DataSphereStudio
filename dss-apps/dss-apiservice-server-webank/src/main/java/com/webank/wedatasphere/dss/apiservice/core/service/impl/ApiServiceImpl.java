...
public class ApiServiceImpl implements ApiService, ApprovalStatusListener {
    ...
    public void save(ApiServiceVo apiService) throws Exception {
        ...
        try {
            ...
            apiVersionVo.setDatasource(apiService.getDatasource());
            ...
            if (params != null && !params.isEmpty()) {
                ...
                for (ParamVo param : params) {
                    ...
                    // 若参数类型为Date，Number不需要用户配置长度限制
                    if (StrUtil.equalsAny(param.getType(), String.valueOf(ParamTypeEnum.NUMBER.getIndex()),
                            String.valueOf(ParamTypeEnum.DATE.getIndex()))
                            && param.getMaxLength() != null) {
                        DSSExceptionUtils.dealErrorException(800034, String.format("%s 参数类型为 %s, 无法自定义长度设置!", param.getName(),
                                ParamTypeEnum.getEnum(Integer.parseInt(param.getType())).getName()), DSSErrorException.class);
                    }

                    // 判断传入的长度必须为正整数
                    if (param.getMaxLength() != null && (param.getMaxLength() < 1 || param.getMaxLength() > 20000)) {
                        DSSExceptionUtils.dealErrorException(800034, String.format("%s 参数长度设置必须在1~20000之间", param.getName()),
        ...
        try {
            if (null != apiService.getTargetServiceId()) {
                ...
                if (maxTargetApiVersionVo.getCreator().equals(apiService.getModifier())) {
                    ...
                    //代理用户
                    apiService.setExecuteUser(StringUtils.isEmpty(apiService.getExecuteUser())
                            ? apiService.getCreator() : apiService.getExecuteUser());
...
//                        maxTargetApiVersionVo.setVersion(version);
                        ...
                        maxTargetApiVersionVo.setDatasource(apiService.getDatasource());
                        ...
                        apiServiceVersionVo.setDatasource(apiService.getDatasource());
...
//                apiServiceVo.setParams(apiServiceParamDao.queryByVersionId(maxApprovedVersion.getId()));
    ...
    public List<ApiServiceVo> queryAvailableSubmitApi(String username, Integer workspaceId) throws ApiServiceQueryException {
        List<ApiServiceVo> apiServiceVoList = apiServiceDao.queryByWorkspaceId(workspaceId, username, null);
    ...
    @Override
    public List<ApiServiceVo> queryByWorkspaceId(Integer workspaceId, String userName, String type) {
        List<ApiServiceVo> result = apiServiceDao.queryByWorkspaceId(workspaceId, userName, type);
        //不需要返回approvalNo
...
//        Map<String, Object> metaDataInfo = ExecuteCodeHelper.getMetaDataInfoByExecute(user, apiService.getContent(), apiService.getMetadata(), apiService.getScriptPath());
        ...
        String executeCode = apiService.getContent();

        if (SQLCheckUtil.isPySpark(apiService.getScriptPath())) {
            executeCode = apiService.getPySparkSql();
        }

        Map<String, Object> metaDataInfo = engineManagerService.getMetadataTableInfos(user, apiService.getType(), executeCode, apiService.getMetadata(), apiService.getScriptPath());
        ...
        String regExp = "^\\[.*";
        if (!Pattern.matches(regExp, StringUtils.strip(metaDataInfoStr))) {
            LOG.error("metaDataInfoStr is {}", metaDataInfoStr);
            throw new ApiExecuteException(80056, "发布或更新数据API失败，解析数据服务脚本中的库表信息出错，错误信息：" + metaDataInfoStr);
        }
...
//            List<String> wrongDbTables = new ArrayList<>();
            for (String item : dbTableNames) {
                ...
                dataMapApplyContentData.setSensitiveLevel(String.valueOf(apiSubmitVo.getSensitive()));
                //代理用户
                ...
                try {
                    ...
                    if (devPrinsAndStatus.isEmpty() || devPrinsAndStatus.get("devPrincipals") == null) {
                        ...
                        // jdbc 数据服务,且未获取到负责人信息
                        if (SQLCheckUtil.isJdbc(apiVersionVo.getScriptPath())
                                && StringUtils.isNotEmpty(apiSubmitVo.getDevPrincipals()) ) {

                            devPrinsAndStatus.put("devPrincipals", apiSubmitVo.getDevPrincipals());
                            dataMapApplyContentData.setProductInfo(apiSubmitVo.getProductInfo());
...
//            }
    ...
    public void addApprovalToDB(ApiServiceVo apiService, ApiVersionVo apiVersionVo, ApiSubmitVo apiSubmitVo) {
        ...
        approvalVo.setSensitiveLevel(apiSubmitVo.getSensitive());
        //代理用户移到api_version表
...
//        tokenManagerVo.setDuration(1000L);
    ...
    public void updateApiServiceBML(String serverUrl) {
        ...
        for (ApiVersionVo apiVersionVo : apiVersionVos) {
            try (InputStream inputStream = oldBmlClient.downloadResource(apiVersionVo.getCreator(), apiVersionVo.getBmlResourceId(), apiVersionVo.getBmlVersion()).inputStream()) {
                if (inputStream == null) continue;
                BmlUploadResponse response = client.uploadResource(apiVersionVo.getCreator(), apiVersionVo.getSource(), inputStream);
    ...
    public List<Map<String, Object>> checkApiServiceBML(String serverUrl) {
        ...
        for (ApiVersionVo apiVersionVo : apiVersionVos) {
            ...
            try (InputStream oldResourceStream = oldBmlClient.downloadResource(apiVersionVo.getCreator(), apiVersionVo.getBmlResourceId(), apiVersionVo.getBmlVersion()).inputStream();
                 InputStream resourceStream = client.downloadResource(apiVersionVo.getCreator(), apiVersionVo.getBmlResourceId(), apiVersionVo.getBmlVersion()).inputStream()) {
                if (oldResourceStream == null || resourceStream == null) continue;
                String oldReadStream = readStream(oldResourceStream);
                String readStream = readStream(resourceStream);
    ...
    @Override
    public boolean isExistsJdbcApi(List<Long> apiId) {

        if (CollectionUtils.isEmpty(apiId)) {
            throw new DSSErrorException(-1, "query apiId is empty");
        }

        return !CollectionUtils.isEmpty(apiServiceDao.queryApiServiceById(apiId, "jdbc"));
    }


    @Override
    public ApiServiceVo checkSubmitApi(List<Long> apiVersionId) {

        ApiServiceVo apiServiceVo = null;

        List<ApiVersionVo> apiVersionVoList = apiServiceVersionDao.selectApiVersionById(apiVersionId);

        List<ApiVersionVo> jdbcApiVersion = apiVersionVoList.stream()
                .filter(apiVersionVo -> SQLCheckUtil.isJdbc(apiVersionVo.getSource()))
                .collect(Collectors.toList());

        // 没有jdbc的api 直接返回true
        if (CollectionUtils.isEmpty(jdbcApiVersion)) {
            return apiServiceVo;
        }

        for (ApiVersionVo apiVersionVo : jdbcApiVersion) {

            String metaDataInfo = apiVersionVo.getMetadataInfo();
            ...
            if (!StringUtils.isBlank(rawMetaDataInfo)) {
                ...
                for (String item : dbTableNames) {
                    ...
                    // 未找到负责人信息
                    if (MapUtils.isEmpty(devPrinsAndStatus) || devPrinsAndStatus.get("devPrincipals") == null) {
                        apiServiceVo = apiServiceDao.queryById(apiVersionVo.getApiId());
                        LOG.info("api service name is {},dbTable is {},tableName is {}, devPrincipals is empty ", apiServiceVo.getName() ,dbTable, tableName);
                        return apiServiceVo;
                    }
