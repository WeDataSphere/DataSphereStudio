...
public class FlowRestfulApi {
    ...
    public Message saveFlow( @RequestBody SaveFlowRequest saveFlowRequest) throws IOException {
        ...
        // v2.3：移除④「开始结束结构」后已无 warn，仅 error（边引用/环路/重名）阻断保存；service 层既有逻辑零改动。
        // forceSave 字段保留（前端可能仍传），后端忽略即可，零风险。
        ...
        if (dagValidation.hasErrors()) {
            ...
            // 用 Message.ok()+status 字段返回，避免 api.fetch 对 Message.error 自动弹 Notice 造成双提示
            return Message.ok()
