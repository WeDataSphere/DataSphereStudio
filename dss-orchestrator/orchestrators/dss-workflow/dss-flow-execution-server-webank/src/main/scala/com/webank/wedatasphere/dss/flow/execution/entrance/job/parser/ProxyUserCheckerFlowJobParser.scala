...
class ProxyUserCheckerFlowJobParser extends FlowEntranceJobParser with Logging {
    ...
    val execByProxyUserEnable = flowEntranceJob.getParams.getOrDefault(DSSCommonUtils.DSS_EXECUTE_BY_PROXY_USER_KEY,"false") match {
      case enable: String => "true" == enable
    }
    val isDevEnv= flowEntranceJob.getParams.getOrDefault(DSSCommonUtils.DSS_LABELS_KEY, DSSCommonUtils.ENV_LABEL_VALUE_DEV) match {
      case label: String => DSSCommonUtils.ENV_LABEL_VALUE_DEV == label
    }
    if(isDevEnv && !execByProxyUserEnable) return
      ...
      case workspace: Workspace =>
        ...
        optionalService.getOptionalOperation(RefProxyUserFetchOperation.OPERATION_NAME) match {
          case operation: RefProxyUserFetchOperation =>
            ...
            val proxyUserList = operation.apply(requestRef).getProxyExecuteUserList
