...
public interface ErrorCodeMapper {
    ...
    @Insert("insert into `dss_error_code_report`(`uri`,`request_body`,`request_headers`,`query_params`,`response_body`,`error_code`,`report_by`,`error_desc`,`create_time`)" +
            "values(#{uri},#{requestBody},#{requestHeaders},#{queryParams},#{responseBody},#{errCode},#{reportBy},#{errorDesc},now())")
    ...
    @Select("<script>" +
            "SELECT * FROM dss_error_code_report " +
            "<where>" +
            "    <if test='reportBy != null'>" +
            "        AND report_by = #{reportBy}" +
            "    </if>" +
            "    <choose>" +
    ...
    List<ErrorCodeReport> getAllErrorReports(
            @Param("reportBy") String reportBy,
