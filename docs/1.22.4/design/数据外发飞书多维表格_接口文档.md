...
```json
{
  ...
  "data": {
    ...
    "taskIds": [9],
    "notifyUsers": ["v_sunpengwang"],
    "message": "全部表外发失败(1): test_1500col_1row: createSheet failed: B2669999 飞书接口调用失败: operationType=创建数据表, code=99992402, msg=field validation failed"
  }
}
```
> 注：失败时 envelope 顶层 `success`/`code`/`message` 可能为通用值（如上例顶层 `message` 仍为"外发进行中"），**状态判定以 `data.status=export_failed` 为准**，**失败原因取 `data.message`**（envelope 顶层 `message` 为通用文案，不可用于定位）。

| 字段 | 类型 | 说明 |
|------|------|------|
| `data.dmId` | string | dm 单号 |
| `data.optype` | string | `table` |
| `data.status` | string | `exporting`（进行中）/ `exported`（完成）/ `export_failed`（外发失败终态） |
...
| `data.message` | string | 外发失败原因（**仅 status=export_failed/failed 时返回**，如"全部表外发失败(1): ... createSheet failed: ..."）；注意区分 envelope 顶层 `message`（通用文案），失败定位以本字段为准 |
...
> **外发失败终态**：当 `status=export_failed`（或 `failed`）时为外发失败终态，DSS 节点直接失败（82007，不重试）；**失败原因随 `data.message` 返回**（DSS 据此打印/抛出异常），各表写入明细仍记服务端审计 `export_result`。
> **以 status 为准**：envelope 顶层 `message` 可能为通用文案（如失败时仍返回"外发进行中，请稍后查询"），业务判定一律以 `data.status` 为准、失败原因以 `data.message` 为准，不依赖 envelope 的 `success`/`code`/`message`。
