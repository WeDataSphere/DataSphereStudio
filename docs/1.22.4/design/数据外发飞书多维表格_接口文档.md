...
```
> 注：失败时 envelope 的 `success`/`code`/`message` 可能为通用值（如上例 `message` 仍为"外发进行中"），判定以 `data.status=export_failed` 为准。
...
> **外发失败终态**：当 `status=export_failed`（或 `failed`）时为外发失败终态，DSS 节点直接失败（82007，不重试）；失败明细写入服务端审计 `export_result`，不随响应返回。
> **以 status 为准**：envelope 的 `message` 可能为通用文案（如失败时仍返回"外发进行中，请稍后查询"），业务判定一律以 `data.status` 为准，不依赖 `success`/`code`/`message`。
