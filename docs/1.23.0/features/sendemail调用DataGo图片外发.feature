...
功能: sendemail 节点调用 DataGo 数据外发图片消息
  ...
  场景: 节点选择发送飞书但 session.token 为空时，sendemail 节点执行失败
    假如 sendemail 节点参数 sendFeishu=true
    而且 wds.dss.appconn.datago.outbound.session.token 为空
    而且 sendemail 节点参数 feishuTo 配置了有效的飞书接收人工号
    当 执行 sendemail 节点
  ...
  场景: ① 受理成功返回 taskId 后进入轮询
    ...
    而且 请求头携带 Authorization: Bearer <session-token> 与 Cookie: dss_user_name=<loginUser>
  ...
  # ============================================================
  # 场景10: recipients 过滤（DataGo 侧拒 v_ 前缀外包与系统用户前缀）
  # ============================================================

  场景: feishuTo 全为 v_ 前缀外包用户时，DataGo 过滤后为空，节点失败
    假如 sendemail 节点参数 sendFeishu=true
    而且 sendemail 节点参数 feishuTo 为"v_sunpengwang;v_test"
    而且 DataGo ① 受理接口返回 code=400 且 message 包含"recipients 不能为空"
    当 执行 sendemail 节点
    那么 抛出 EmailSendFailedException(81002)
    而且 sendemail 节点状态为失败
    而且 不重试（非 502/504）

  场景: feishuTo 含 v_ 前缀与有效工号时，仅有效接收人收到飞书消息
    假如 sendemail 节点参数 sendFeishu=true
    而且 sendemail 节点参数 feishuTo 为"burdezhang;v_sunpengwang"
    而且 DataGo ① 受理接口返回 success=true 且 taskId=2048
    而且 DataGo ② 轮询接口返回 status=exported
    当 执行 sendemail 节点
    那么 DataGo ① 受理请求 recipients 原样提交为 ["burdezhang","v_sunpengwang"]
    而且 DataGo 侧过滤 v_ 前缀后仅 burdezhang 收到飞书图片消息
    而且 sendemail 节点状态为成功

  场景: feishuTo 含系统用户前缀时，DataGo 过滤系统用户
    假如 sendemail 节点参数 sendFeishu=true
    而且 sendemail 节点参数 feishuTo 为"hadoopadmin;burdezhang"
    而且 DataGo ① 受理成功且 ② 轮询 exported
    当 执行 sendemail 节点
    那么 DataGo 过滤 hadoop 前缀系统用户后仅 burdezhang 收到飞书消息

  场景: session-token 过期时 DataGo 返回 401，节点失败
    假如 sendemail 节点参数 sendFeishu=true
    而且 wds.dss.appconn.datago.outbound.session.token 已过期
    而且 DataGo ① 受理接口返回 401 且 message 包含"无效的 session-token 鉴权"
    而且 sendemail 节点参数 feishuTo 配置了有效的飞书接收人工号
    当 执行 sendemail 节点
    那么 邮件发送成功
    而且 抛出 EmailSendFailedException(81002)
